...
final class AdminController
{
    ...
    private const ACTION_DOWNLOAD_SITE_TRANSFER_EXPORT_BUNDLE = 'tasty_fonts_download_site_transfer_export_bundle';
    private const ACTION_DOWNLOAD_SUPPORT_BUNDLE = 'tasty_fonts_download_support_bundle';
    private const ACTION_IMPORT_SITE_TRANSFER_BUNDLE = 'tasty_fonts_import_site_transfer_bundle';
    private const ACTION_RENAME_SITE_TRANSFER_EXPORT_BUNDLE = 'tasty_fonts_rename_site_transfer_export_bundle';
    private const ACTION_PROTECT_SITE_TRANSFER_EXPORT_BUNDLE = 'tasty_fonts_protect_site_transfer_export_bundle';
    private const ACTION_DELETE_SITE_TRANSFER_EXPORT_BUNDLE = 'tasty_fonts_delete_site_transfer_export_bundle';
    private const ACTION_CREATE_ROLLBACK_SNAPSHOT = 'tasty_fonts_create_rollback_snapshot';
    ...
    private readonly RoleFamilyCatalogBuilder $roleFamilyCatalogBuilder;
    ...
    public function enqueueAssets(string $hookSuffix): void
    {
        ...
        wp_localize_script(
            ...
            [
                ...
                'globalFallbacks' => $runtimeState['globalFallbacks'],
    ...
    public function handleAdminActions(): void
    {
        ...
        if ($this->handleDownloadSiteTransferExportBundleAction()) {
            return;
        }

        if ($this->handleDownloadSupportBundleAction()) {
            return;
        }
        ...
        if ($this->handleResetFamilyFallbacksToGlobalAction()) {
            return;
        }

        if ($this->handleResetFallbacksToPluginDefaultsAction()) {
            return;
        }
        ...
        if ($this->handleCreateRollbackSnapshotAction()) {
            return;
        }

        if ($this->handleRestoreRollbackSnapshotAction()) {
            return;
        }

        if ($this->handleRenameRollbackSnapshotAction()) {
            return;
        }

        if ($this->handleDeleteRollbackSnapshotAction()) {
            return;
        }
        ...
        if ($this->handleProtectSiteTransferExportBundleAction()) {
            return;
        }
    ...
    private function fontImportWorkflowEnabled(string $workflow): bool
    {
        ...
        if ($workflow === 'custom_css_url' && !$this->experimentalFontImportWorkflowsAvailable($settings)) {
            return false;
        }
     ...
     * @return Payload|WP_Error
     */
    public function publishRoleDraftValues(array $roleValues): array|WP_Error
    {
        $catalog = $this->catalog->getCatalog();
        $availableFamilies = $this->buildSelectableFamilyNames($catalog);
        $settingsBefore = $this->settings->getSettings();
        $wasAppliedSitewide = !empty($settingsBefore['auto_apply_roles']);

        $roles = $roleValues === []
            ? $this->settings->getRoles($availableFamilies)
            : $this->settings->saveRoles(
                $this->sanitizeRoleValues($roleValues, $catalog),
                $availableFamilies
            );
        ...
        $liveRoles = $this->settings->getAppliedRoles($availableFamilies);
        $this->library->syncLiveRolePublishStates($liveRoles, true);
        ...
        $this->assets->refreshGeneratedAssets(false);
        $this->assets->ensureGeneratedCssFile(false);
        $generatedCss = $this->assets->getCss();

        $message = $this->buildRolesSavedMessage('apply', $roles, $liveRoles, true);
        ...
        $this->log->add(
            ...
            $this->buildActivityLogContext(
                ...
                [
                    'outcome' => 'success',
                    'status_label' => __('Published', 'tasty-fonts'),
                    'source' => __('Roles', 'tasty-fonts'),
                    'details' => $this->buildRoleActivityDetails($roles, $liveRoles, true),
                ]
            )
        );
        ...
        return [
            'message' => $message,
            'roles' => $roles,
            'applied_roles' => $liveRoles,
            'apply_everywhere' => true,
            'preview_baseline_source' => 'live_sitewide',
            'preview_baseline_label' => __('Live sitewide', 'tasty-fonts'),
            'role_deployment' => $this->buildRoleDeploymentContext($roles, $liveRoles, true, $settings),
            'role_family_catalog' => $this->buildRoleFamilyCatalog($catalog),
            'global_fallbacks' => $this->globalFallbackPayload($settings),
            'canvas_refresh' => [
                'mode' => 'runtime_styles',
    ...
    public function saveSettingsValues(array $submittedValues): array|WP_Error
    {
        ...
        $settingsInput = $this->gateExperimentalSettingsInput($settingsInput, $previousSettings);
        ...
        if (($previousSettings['snapshot_retention_limit'] ?? null) !== ($savedSettings['snapshot_retention_limit'] ?? null)) {
            $this->snapshots->pruneSnapshotsToRetentionLimit();
        }

        if (($previousSettings['site_transfer_export_retention_limit'] ?? null) !== ($savedSettings['site_transfer_export_retention_limit'] ?? null)) {
            $this->siteTransfer->pruneExportBundlesToRetentionLimit();
        }
        ...
        $outputTogglePreviews = $this->buildOutputTogglePreviews($roles, $savedSettings, $catalog, $appliedRoles);
        ...
        if ($clearGoogleKey) {
            ...
            return [
                ...
                'output_toggle_previews' => $outputTogglePreviews,
     ...
     * @return Payload|WP_Error
     */
    public function stageSiteTransferBundle(array $uploadedFile, string $freshGoogleApiKey = ''): array|WP_Error
    {
        ...
        $message = sprintf(
            __('Dry run succeeded. Ready to import %1$d famil%2$s and %3$d file%4$s. Use Import Bundle to continue.', 'tasty-fonts'),
            $familyCount,
            $familyCount === 1 ? 'y' : 'ies',
            $fileCount,
            $fileCount === 1 ? '' : 's'
     ...
     */
    private function validateFreshTransferGoogleApiKey(string $freshGoogleApiKey): array|WP_Error
    {
        $freshGoogleApiKey = trim($freshGoogleApiKey);

        if ($freshGoogleApiKey === '') {
            return [
                'state' => 'empty',
                'message' => '',
            ];
     ...
     * @param array{state: string, message: string} $googleApiKeyValidation
     * @return Payload|WP_Error
     */
    private function applyFreshTransferGoogleApiKeyStatus(array|WP_Error $result, array $googleApiKeyValidation): array|WP_Error
    {
        if (is_wp_error($result) || $googleApiKeyValidation['state'] !== 'valid') {
            return $result;
        }

        $this->settings->saveGoogleApiKeyStatus(
            $googleApiKeyValidation['state'],
            $googleApiKeyValidation['message']
        );

        $result['settings'] = $this->settings->getSettings();
    ...
    public function refreshLibraryView(array $input): array
    {
        ...
        return [
            ...
            'global_fallbacks' => $runtimeState['globalFallbacks'],
    ...
    private function renderFamilyCardDetailsHtml(array $family, array $view): string
    {
        ...
        $globalFallbackSettings = $this->globalFallbackSettingsFromView($view);
        ...
        $renderer->renderFamilyCardDetails([
            ...
            'global_fallback_settings' => $globalFallbackSettings,
        ]);
     ...
     */
    private function globalFallbackSettingsFromView(array $view): array
    {
        return [
            'fallback_heading' => FontUtils::sanitizeFallback($this->stringValue($view, 'fallbackHeading', FontUtils::DEFAULT_ROLE_SANS_FALLBACK)),
            'fallback_body' => FontUtils::sanitizeFallback($this->stringValue($view, 'fallbackBody', FontUtils::DEFAULT_ROLE_SANS_FALLBACK)),
            'fallback_monospace' => FontUtils::sanitizeFallback($this->stringValue($view, 'fallbackMonospace', FontUtils::DEFAULT_ROLE_MONOSPACE_FALLBACK)),
        ];
    }
    ...
    private function buildAdminRuntimeStatePayload(
        ...
        return [
            ...
            'variableFontsEnabled' => !empty($settings['variable_fonts_enabled']),
            'customCssUrlImportsEnabled' => $this->experimentalFontImportWorkflowsAvailable($settings) && !empty($settings['custom_css_url_imports_enabled']),
            'roleFamilyCatalog' => $this->buildRoleFamilyCatalog($catalog),
            'globalFallbacks' => $this->globalFallbackPayload($settings),
            'previewBootstrap' => [
                'roles' => $roles,
    ...
    private function handleResetFamilyFallbacksToGlobalAction(): bool
    {
        if (!isset($_POST['tasty_fonts_reset_family_fallbacks_to_global'])) {
            return false;
        }

        check_admin_referer('tasty_fonts_reset_family_fallbacks_to_global');

        $result = $this->maintenanceActions->resetFamilyFallbacksToGlobal();

        if (is_wp_error($result)) {
            $this->redirectWithError($result->get_error_message());
        }

        $this->redirectWithSuccess($this->stringValue($result, 'message', __('Custom fallbacks deleted.', 'tasty-fonts')));
    }

    private function handleResetFallbacksToPluginDefaultsAction(): bool
    {
        if (!isset($_POST['tasty_fonts_reset_fallbacks_to_plugin_defaults'])) {
            return false;
        }

        check_admin_referer('tasty_fonts_reset_fallbacks_to_plugin_defaults');

        $result = $this->maintenanceActions->resetFallbacksToPluginDefaults();

        if (is_wp_error($result)) {
            $this->redirectWithError($result->get_error_message());
        }

        $this->redirectWithSuccess($this->stringValue($result, 'message', __('Fallback defaults restored.', 'tasty-fonts')));
    }
    ...
    private function handleCreateRollbackSnapshotAction(): bool
    {
        if (!isset($_POST[self::ACTION_CREATE_ROLLBACK_SNAPSHOT])) {
            return false;
        }

        check_admin_referer(self::ACTION_CREATE_ROLLBACK_SNAPSHOT);

        $result = $this->createRollbackSnapshot('manual');

        if (is_wp_error($result)) {
            $this->redirectWithError($result->get_error_message());
        }

        $this->redirectWithSuccess($this->stringValue($result, 'message', __('Rollback snapshot created.', 'tasty-fonts')));
    }

    private function handleRestoreRollbackSnapshotAction(): bool
    {
        if (!isset($_POST[self::ACTION_RESTORE_ROLLBACK_SNAPSHOT])) {
            return false;
        }

        check_admin_referer(self::ACTION_RESTORE_ROLLBACK_SNAPSHOT);

        $snapshotId = $this->getPostedText('tasty_fonts_snapshot_id');
        $result = $this->restoreRollbackSnapshot($snapshotId);

        if (is_wp_error($result)) {
            $this->redirectWithError($result->get_error_message());
        }

        $this->redirectWithSuccess($this->stringValue($result, 'message', __('Rollback snapshot restored.', 'tasty-fonts')));
    }

    private function handleRenameRollbackSnapshotAction(): bool
    {
        if (!isset($_POST[self::ACTION_RENAME_ROLLBACK_SNAPSHOT])) {
            return false;
        }

        check_admin_referer(self::ACTION_RENAME_ROLLBACK_SNAPSHOT);

        $snapshotId = $this->getPostedText('tasty_fonts_snapshot_id');
        $label = $this->getPostedText('tasty_fonts_snapshot_label');
        $result = $this->renameRollbackSnapshot($snapshotId, $label);

        if (is_wp_error($result)) {
            $this->redirectWithError($result->get_error_message());
        }

        $this->redirectWithSuccess($this->stringValue($result, 'message', __('Rollback snapshot renamed.', 'tasty-fonts')));
    }

    private function handleDeleteRollbackSnapshotAction(): bool
    {
        if (!isset($_POST[self::ACTION_DELETE_ROLLBACK_SNAPSHOT])) {
            return false;
        }

        check_admin_referer(self::ACTION_DELETE_ROLLBACK_SNAPSHOT);

        $snapshotId = $this->getPostedText('tasty_fonts_snapshot_id');
        $result = $this->deleteRollbackSnapshot($snapshotId);
    ...
    private function handleRenameSiteTransferExportBundleAction(): bool
    {
        ...
        check_admin_referer(self::ACTION_RENAME_SITE_TRANSFER_EXPORT_BUNDLE);

        $exportId = $this->getPostedText('tasty_fonts_export_bundle_id');
        $label = $this->getPostedText('tasty_fonts_export_bundle_label');
        $result = $this->renameSiteTransferExportBundle($exportId, $label);

        if (is_wp_error($result)) {
            $this->redirectWithError($result->get_error_message());
        }

        $this->redirectWithSuccess($this->stringValue($result, 'message', __('Export bundle renamed.', 'tasty-fonts')));
    }

    private function handleProtectSiteTransferExportBundleAction(): bool
    {
        if (!isset($_POST[self::ACTION_PROTECT_SITE_TRANSFER_EXPORT_BUNDLE])) {
            return false;
        }

        check_admin_referer(self::ACTION_PROTECT_SITE_TRANSFER_EXPORT_BUNDLE);

        $exportId = $this->getPostedText('tasty_fonts_export_bundle_id');
        $protected = $this->getPostedText('tasty_fonts_export_bundle_protected') === '1';
        $result = $this->setSiteTransferExportBundleProtected($exportId, $protected);

        if (is_wp_error($result)) {
            $this->redirectWithError($result->get_error_message());
        }

        $this->redirectWithSuccess($this->stringValue($result, 'message', $protected ? __('Export bundle protected.', 'tasty-fonts') : __('Export bundle unprotected.', 'tasty-fonts')));
    }

    private function handleDeleteSiteTransferExportBundleAction(): bool
    {
        if (!isset($_POST[self::ACTION_DELETE_SITE_TRANSFER_EXPORT_BUNDLE])) {
            return false;
        }

        check_admin_referer(self::ACTION_DELETE_SITE_TRANSFER_EXPORT_BUNDLE);

        $exportId = $this->getPostedText('tasty_fonts_export_bundle_id');
        $result = $this->deleteSiteTransferExportBundle($exportId);
                ...
                array_filter(
                    [
                        ...
                        'monospace_fallback' => isset($_POST['tasty_fonts_monospace_fallback'])
                            ? $this->getPostedFallback('tasty_fonts_monospace_fallback', FontUtils::DEFAULT_ROLE_MONOSPACE_FALLBACK)
                            : null,
    ...
    private function handleDownloadSiteTransferBundleAction(): bool
    {
        ...
        readfile($path);
        if (empty($bundle['retained'])) {
            @unlink($path);
        }
        exit;
    }

    private function handleDownloadSiteTransferExportBundleAction(): bool
    {
        if (!self::isPluginPageSlug($this->getQueryText('page', self::MENU_SLUG)) || !isset($_GET[self::ACTION_DOWNLOAD_SITE_TRANSFER_EXPORT_BUNDLE])) {
            return false;
        }

        check_admin_referer(self::ACTION_DOWNLOAD_SITE_TRANSFER_EXPORT_BUNDLE);

        $bundle = $this->siteTransfer->exportBundleDownload($this->getQueryText('tasty_fonts_export_bundle_id'));
        ...
        if ($path === '' || !is_readable($path)) {
            $this->redirectWithError(__('The saved export bundle could not be prepared for download.', 'tasty-fonts'));
        }
    ...
    private function handleDownloadSupportBundleAction(): bool
    {
        if (!self::isPluginPageSlug($this->getQueryText('page', self::MENU_SLUG)) || !isset($_GET[self::ACTION_DOWNLOAD_SUPPORT_BUNDLE])) {
            return false;
        }

        check_admin_referer(self::ACTION_DOWNLOAD_SUPPORT_BUNDLE);

        $result = $this->buildSupportBundle();
        ...
        if ($path === '' || !is_readable($path)) {
            $this->redirectWithError(__('The support bundle could not be prepared for download.', 'tasty-fonts'));
        }

        $filename = sanitize_file_name($this->stringValue($bundle, 'filename', 'tasty-fonts-support.zip'));
        $contentType = trim($this->stringValue($bundle, 'content_type', 'application/zip'));
        $size = is_file($path) ? (int) filesize($path) : 0;
     ...
     * @return array<string, mixed>
     */
    private function buildOutputTogglePreviews(
        array $roles,
        array $settings,
        array $catalog = [],
        array $appliedRoles = []
    ...
    {
        return $this->pageContextBuilder->buildOutputTogglePreviews($roles, $settings, $catalog, $appliedRoles);
    }
    ...
    private function buildSiteTransferContext(array $logs = []): array
    {
        ...
        return [
            ...
            'support_bundle_url' => $this->buildSupportBundleDownloadUrl(),
            'export_download_action_field' => self::ACTION_DOWNLOAD_SITE_TRANSFER_EXPORT_BUNDLE,
            ...
            'snapshot_action_field' => self::ACTION_CREATE_ROLLBACK_SNAPSHOT,
     ...
     * @return PayloadList
     */
    private function buildSiteTransferExportBundlesContext(): array
    {
        $bundles = [];

        foreach ($this->siteTransfer->listExportBundles() as $bundle) {
            $bundle['download_url'] = $this->buildSiteTransferExportBundleDownloadUrl((string) $bundle['id']);
            $bundles[] = $bundle;
        }
    ...
    private function buildSettingsSavedMessage(array $before, array $after): string
    {
        ...
        if (!empty($before['etch_quick_roles_panel_enabled']) !== !empty($after['etch_quick_roles_panel_enabled'])) {
            $changes[] = !empty($after['etch_quick_roles_panel_enabled'])
                ? __('Etch quick role panel enabled', 'tasty-fonts')
                : __('Etch quick role panel disabled', 'tasty-fonts');
        }
    ...
    private function settingsChangeRequiresReload(array $before, array $after): bool
    {
        return !empty($before['training_wheels_off']) !== !empty($after['training_wheels_off'])
            ...
            || !empty($before['etch_quick_roles_panel_enabled']) !== !empty($after['etch_quick_roles_panel_enabled'])
     ...
     */
    private function gateExperimentalSettingsInput(array $settingsInput, array $previousSettings): array
    {
        $updateChannel = array_key_exists('update_channel', $settingsInput)
            ? $this->stringValue($settingsInput, 'update_channel', SettingsRepository::UPDATE_CHANNEL_STABLE)
            : $this->stringValue($previousSettings, 'update_channel', SettingsRepository::UPDATE_CHANNEL_STABLE);

        if (!$this->experimentalFontImportWorkflowsAvailable(['update_channel' => $updateChannel])) {
            $settingsInput['custom_css_url_imports_enabled'] = false;
        }
     ...
     */
    private function experimentalFontImportWorkflowsAvailable(array $settings): bool
    {
        return in_array(
            $this->stringValue($settings, 'update_channel', SettingsRepository::UPDATE_CHANNEL_STABLE),
            [
    ...
    private function validateOutputSettingsInput(array $settingsInput, array $previousSettings): ?WP_Error
    {
        ...
        foreach ($outputLayerFields as $field) {
        ...
        if (
            array_key_exists('output_quick_mode_preference', $settingsInput)
            && !array_intersect($outputLayerFields, array_keys($settingsInput))
        ) {
            switch ($this->stringValue($settingsInput, 'output_quick_mode_preference')) {
                case 'minimal':
                    $nextSettings['minimal_output_preset_enabled'] = true;
                    $nextSettings['class_output_enabled'] = false;
                    $nextSettings['per_variant_font_variables_enabled'] = true;
                    break;
                ...
                case 'classes':
                    $nextSettings['minimal_output_preset_enabled'] = false;
                    $nextSettings['class_output_enabled'] = true;
                    $nextSettings['per_variant_font_variables_enabled'] = false;
                    break;
    ...
    private function preserveUnavailableIntegrationSettings(array $settingsInput): array
    {
        ...
        if (!$this->etchIntegrationAvailable()) {
            ...
            unset($settingsInput['etch_quick_roles_panel_enabled']);
        }
     ...
     * @return array<string, string>
     */
    private function globalFallbackPayload(array $settings): array
    {
        return [
            'heading' => FontUtils::sanitizeFallback(FontUtils::scalarStringValue($settings['fallback_heading'] ?? FontUtils::DEFAULT_ROLE_SANS_FALLBACK)),
            'body' => FontUtils::sanitizeFallback(FontUtils::scalarStringValue($settings['fallback_body'] ?? FontUtils::DEFAULT_ROLE_SANS_FALLBACK)),
            'monospace' => FontUtils::sanitizeFallback(FontUtils::scalarStringValue($settings['fallback_monospace'] ?? FontUtils::DEFAULT_ROLE_MONOSPACE_FALLBACK)),
        ];
    }
    ...
    {
        return $this->roleFamilyCatalogBuilder->build($catalog, $this->settings->getSettings());
    }
    ...
    private function initializeDetectedIntegrations(bool $queueToasts = true, bool $returnErrors = false): array|WP_Error
    {
        ...
        if (
            ...
            && empty($settings['acss_font_role_sync_opted_out'])
            && empty($settings['acss_font_role_sync_applied'])
            && $this->hasAcssBackupValues($settings)
        ) {
    ...
    private function reconcileAcssIntegrationDrift(): void
    {
        ...
        // Leave the saved sync state intact so the settings UI can show the
        // current Automatic.css values and offer a deliberate reapply action.
    }
    ...
    private function buildSiteTransferExportBundleDownloadUrl(string $exportId): string
    {
        return add_query_arg(
            [
                'page' => self::MENU_SLUG,
                'tf_page' => self::PAGE_DIAGNOSTICS,
                'tf_studio' => 'transfer',
                self::ACTION_DOWNLOAD_SITE_TRANSFER_EXPORT_BUNDLE => '1',
                'tasty_fonts_export_bundle_id' => sanitize_key($exportId),
                '_wpnonce' => wp_create_nonce(self::ACTION_DOWNLOAD_SITE_TRANSFER_EXPORT_BUNDLE),
            ],
            admin_url('admin.php')
        );
    }

    private function buildSupportBundleDownloadUrl(): string
    {
        return add_query_arg(
            [
                'page' => self::MENU_SLUG,
                'tf_page' => self::PAGE_DIAGNOSTICS,
                'tf_studio' => 'transfer',
                self::ACTION_DOWNLOAD_SUPPORT_BUNDLE => '1',
                '_wpnonce' => wp_create_nonce(self::ACTION_DOWNLOAD_SUPPORT_BUNDLE),
            ],
            admin_url('admin.php')
        );
    }
