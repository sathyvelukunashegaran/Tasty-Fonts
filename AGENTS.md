...
- Quality and release helpers: `bin/lint-design`, `bin/run-jscpd`, `bin/setup-git-hooks`, `bin/release`, `bin/nightly-version`, `tests/bin/release-scripts.test.sh`
...
6. Expect `composer phpstan`, optional Stylelint when npm dependencies are installed, Impeccable design linting in CI, and `jscpd` to run on commit through the shared pre-commit hook once `bin/setup-git-hooks` has been run in the clone.
...
```bash
composer install && npm ci && composer phpstan && npm run lint:css && npm run lint:design && find . -name '*.php' -not -path './output/*' -not -path './tmp/*' -not -path './vendor/*' -print0 | xargs -0 -n1 php -l && php tests/run.php && node --test tests/js/*.test.cjs && bash tests/bin/release-scripts.test.sh && bin/run-jscpd
```
